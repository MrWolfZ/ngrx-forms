...
validateBookExists$: Observable<Action> = this.actions$
  ...
  .switchMap(a =>
    this.http.get(`api/books/search/${a.searchTerm}`)
      .map(resp =>
        resp.status === 404
          ? new SetAsyncErrorAction(a.controlId, "exists", true) // controlId may either be sent with the action or obtained from the store via withLatestFrom
