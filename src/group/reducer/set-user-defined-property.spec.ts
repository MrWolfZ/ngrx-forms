...
describe(`form group ${setUserDefinedPropertyReducer.name}`, () => {
  ...
  it('should not update state user defined properties if same', () => {
    ...
    const resultState = setUserDefinedPropertyReducer<typeof state.value>(state,SetUserDefinedPropertyAction(FORM_CONTROL_ID, prop, value));
  ...
  it('should update state user defined properties for children', () => {
    const prop = 'prop';
    const value = 12;
    const resultState = setUserDefinedPropertyReducer(INITIAL_STATE,SetUserDefinedPropertyAction(FORM_CONTROL_INNER_ID, prop, value));
