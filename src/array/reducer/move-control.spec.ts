...
describe(`form array ${moveControlReducer.name}`, () => {
  ...
  it('should move controls forward', () => {
    let action = MoveArrayControlAction(FORM_CONTROL_ID, 2, 5);
    ...
    action = MoveArrayControlAction(FORM_CONTROL_ID, 0, 3);
  ...
  it('should move controls backwards', () => {
    let action = MoveArrayControlAction(FORM_CONTROL_ID, 5, 2);
    let resultState = moveControlReducer(testArrayState, action);
    expect(resultState).not.toBe(testArrayState);
    expect(resultState.controls).not.toBe(testArrayState.controls);
    expect(resultState.value).toEqual([0, 1, 5, 2, 3, 4]);

    action = MoveArrayControlAction(FORM_CONTROL_ID, 3, 0);
    resultState = moveControlReducer(testArrayState, action);
  ...
  it('should throw an error for negative or too large indices', () => {
    expect(() =>
      moveControlReducer(
        ...
        MoveArrayControlAction(FORM_CONTROL_ID, 0, INITIAL_STATE_NESTED_GROUP.controls.length))
    ).toThrowError();
    expect(() =>
      moveControlReducer(
        INITIAL_STATE_NESTED_GROUP,
        MoveArrayControlAction(FORM_CONTROL_ID, INITIAL_STATE_NESTED_GROUP.controls.length, 0))
    ).toThrowError();
    expect(() =>
      moveControlReducer(
        INITIAL_STATE_NESTED_GROUP,
        MoveArrayControlAction(FORM_CONTROL_ID, INITIAL_STATE_NESTED_GROUP.controls.length, -3))
    ).toThrowError();
    expect(() =>
      moveControlReducer(
        INITIAL_STATE_NESTED_GROUP,
        MoveArrayControlAction(FORM_CONTROL_ID, -1, 0))
    ).toThrowError();
  });

  it('should return the state on a 0 move', () => {
    const action = MoveArrayControlAction(FORM_CONTROL_ID, 1, 1);
  ...
  it('should return the state if applied on a different state ID', () => {
    const action = MoveArrayControlAction(FORM_CONTROL_0_ID, 0, 1);
    const resultState = moveControlReducer(testArrayState, action);
    expect(resultState).toBe(testArrayState);
  });

  it('should update nested group IDs after a move', () => {
    const action = MoveArrayControlAction(FORM_CONTROL_ID, 0, 1);
  ...
  it('should mark the array as dirty', () => {
    const action = MoveArrayControlAction(FORM_CONTROL_ID, 3, 1);
    const resultState = moveControlReducer(testArrayState, action);
    expect(resultState).not.toBe(testArrayState);
    expect(resultState.isDirty).toEqual(true);
  });
