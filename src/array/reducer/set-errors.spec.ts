...
describe(`form array ${setErrorsReducer.name}`, () => {
  ...
  it('should aggregate child errors', () => {
    const errors = { required: true };
    const resultState = setErrorsReducer(INITIAL_STATE, SetErrorsAction(FORM_CONTROL_0_ID, errors));
  ...
  it('should aggregate child errors for group children', () => {
    const errors = { required: true };
    const resultState = setErrorsReducer(INITIAL_STATE_NESTED_GROUP, SetErrorsAction(FORM_CONTROL_0_ID, errors));
    expect(resultState.errors).toEqual({ _0: errors });
    expect(resultState.isValid).toEqual(false);
    expect(resultState.isInvalid).toEqual(true);
  });
  ...
  it('should aggregate child errors for array children', () => {
    const errors = { required: true };
    const resultState = setErrorsReducer(INITIAL_STATE_NESTED_ARRAY, SetErrorsAction(FORM_CONTROL_0_ID, errors));
    expect(resultState.errors).toEqual({ _0: errors });
    expect(resultState.isValid).toEqual(false);
    expect(resultState.isInvalid).toEqual(true);
  });
  ...
  it('should aggregate multiple child errors', () => {
    ...
    const errors2 = { min: 0 };
    let resultState = setErrorsReducer(INITIAL_STATE, SetErrorsAction(FORM_CONTROL_0_ID, errors1));
    resultState = setErrorsReducer(resultState, SetErrorsAction(FORM_CONTROL_1_ID, errors2));
  ...
  it('should track own errors and child errors when child errors are changed', () => {
    ...
    const errors2 = { min: 0 };
    const resultState = setErrorsReducer(state, SetErrorsAction(FORM_CONTROL_0_ID, errors2));
    expect(resultState.errors).toEqual({ ...errors1, _0: errors2 });
  });
});
