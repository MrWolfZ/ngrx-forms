...
describe(`form array ${swapControlReducer.name}`, () => {
  ...
  it('should swap controls forwards', () => {
    const action = SwapArrayControlAction(FORM_CONTROL_ID, 0, 2);
  ...
  it('should swap controls backwards', () => {
    const action = SwapArrayControlAction(FORM_CONTROL_ID, 5, 1);
    const resultState = swapControlReducer(testArrayState, action);
  ...
  it('should throw on out of bound or negative indices', () => {
    expect(() => swapControlReducer(
      ...
      SwapArrayControlAction(FORM_CONTROL_ID, 0, INITIAL_STATE_NESTED_GROUP.controls.length))
    ).toThrow();
    expect(() => swapControlReducer(
      INITIAL_STATE_NESTED_GROUP,
      SwapArrayControlAction(FORM_CONTROL_ID, INITIAL_STATE_NESTED_GROUP.controls.length, 0))
    ).toThrow();
    expect(() => swapControlReducer(
      INITIAL_STATE_NESTED_GROUP,
      SwapArrayControlAction(FORM_CONTROL_ID, -3, INITIAL_STATE_NESTED_GROUP.controls.length))
    ).toThrow();
    expect(() => swapControlReducer(
      INITIAL_STATE_NESTED_GROUP,
      SwapArrayControlAction(FORM_CONTROL_ID, INITIAL_STATE_NESTED_GROUP.controls.length, -2))
    ).toThrow();
  });

  it('should update deeply nested child IDs after a swap', () => {
    const action = SwapArrayControlAction(FORM_CONTROL_ID, 0, 1);
  ...
  it('should return the state unmodified if no element was moved', () => {
    const action = SwapArrayControlAction(FORM_CONTROL_ID, 1, 1);
    const resultState = swapControlReducer(testArrayState, action);
  ...
  it('should return the state if applied on a different state ID', () => {
    const action = SwapArrayControlAction(FORM_CONTROL_0_ID, 0, 1);
    const resultState = swapControlReducer(testArrayState, action);
    expect(resultState).toBe(testArrayState);
  });
  ...
  it('should mark the array as dirty', () => {
    const action = SwapArrayControlAction(FORM_CONTROL_ID, 2, 1);
    const resultState = swapControlReducer(testArrayState, action);
    expect(resultState.isDirty).toEqual(true);
  });
});
